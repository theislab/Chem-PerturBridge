...
To standardize you just need to convert dataset column content or column names to the specific schema described [here](https://github.com/theislab/Chem-PerturBridge/blob/main/docs/Format_Pseudobulk.ipynb).
...
The most important columns to keep an eye on are `plate`, `well`, `cell_type`, `perturbagen`, `pert_type`, `is_control`, `pert_dose_uM`, `pert_time_h`, `pubchem_cid`. Description of them is located [here](https://github.com/theislab/Chem-PerturBridge/blob/main/docs/Format_Pseudobulk.ipynb).
...
Note, to look up `pubchem_cid` for bulk data you can include `lookup_pubchem_cids` from [`pubchem.py`](https://github.com/theislab/Chem-PerturBridge/blob/main/src/pseudobulking/common/pubchem.py) module into your processing/assembling pipeline.
