  	...
  	case $opt in
		h)
			echo "Run: $0 [-s] [-j] [-h] [-g] [-P] [-f] [-q] [-n] -d (dataset: ${VALID_CHOICES[*]}) -p (parameters: ${DEG_PARAMETERS[*]})"
                        ...
                        echo "  -g Use GPU QOS/partition; set GPU_QOS/GPU_PARTITION to override, default=CPU"
                        echo "  -P Use preemptible QOS for DEG step; set PREEMPTIBLE_QOS/GPU_PREEMPTIBLE_QOS to override, default=false"
                        echo "  -h Help option"
