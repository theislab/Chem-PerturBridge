...
def download_save_from_lamindb(key_adata: str = None,
    ...
    if (key_adata) and (path2adata):
        ...
        if not os.path.isfile(path2adata):
            ...
            if subsampling:
                idx = np.random.choice(
                    adata.obs.index, replace=False, size=subsample_size)
        ...
        logger.info(
            'Skipped downloading an anndata file as some arguments are empty')
    ...
    if (key_obs) and (path2obs):
        ...
        logger.info(
            'Skipped downloading an .obs file as some arguments are empty')
    ...
    if (key_var) and (path2var):
        ...
        logger.info(
            'Skipped downloading an .var file as some arguments are empty')
...
def main():
    ...
    def check_input_output_sub_args(d_args: Dict[str, Optional[str]],
        ...
        for i, isub_args in enumerate(required_sub_args['input']):
            ...
            if d_args['input'].get(isub_args) and (not d_args['output'].get(osub_args)):
                raise Exception(
                    f'Both {isub_args} and {osub_args} sub-args should be specified')
    ...
    download_save_from_lamindb(**{**d_args['input'],
                               **d_args['output']},
                               subsampling=d_args['subsampling'])
